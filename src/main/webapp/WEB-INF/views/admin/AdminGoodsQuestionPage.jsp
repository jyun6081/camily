												...
												<c:forEach items="${goodsQnAList}" var="goodsQnAInfo">
													<div class="p-b-68" id="${goodsQnAInfo.gqcode}">
														...
														<div id="${goodsQnAInfo.gqcode}_question">
															...
															<textarea class="stext-102 cl6" id="${goodsQnAInfo.gqcode}_questionContents" name="questionContents" style="width: 100%; resize: none;" readonly="readonly">${goodsQnAInfo.gqcontents}</textarea>
														</div>
														<!-- 답글 -->
																...
																<c:when test="${goodsQnAInfo.gwcode == null}">
																	<div class="row p-b-25">
																		<div class="col-12 p-b-5">
																			...
																			<textarea class="size-110 bor8 stext-102 cl2 p-lr-20 p-tb-10" id="${goodsQnAInfo.gqcode}_gwcontents" name="gwcontents" style="resize: none;"></textarea>
																...
																<c:otherwise>
																	<div class="flex-w flex-t">
																		...
																		<div class="size-207">
																			...
																			<textarea class="stext-102 cl6" id="${goodsQnAInfo.gwcode}_answerForm" name="answer" style="width: 100%; resize: none;" readonly="readonly">${goodsQnAInfo.gwcontents}</textarea>
																		</div>
																	</div>
																</c:otherwise>
															</c:choose>
	...
	<script>
		...
		function gqAnswerSubmit(gqcode){
			...
			success: function(result){
				...
				output += '<textarea class="stext-102 cl6" id="answer" name="answer" style="width: 100%; resize: none;" readonly="readonly">' + result.gwcontents + '</textarea>';
				output += '</div>';
				output += '</div>';
		...
		function modifyAnswerForm(gwcode){
			...
			$("#" + gwcode + "_answerForm").addClass("size-110 bor8 stext-102 cl2 p-lr-20 p-tb-10");

			var output = "";
			output += '<button class="btn btn-primary m-r-10" onclick="modifyAnswer(\'' + gwcode + '\')">확인</button>';
			output += '<button class="btn btn-danger" onclick="cancelModify(\'' + gwcode + '\')">취소</button>'
			$("#"+gwcode+"_modifyAnswerBtn").html(output);
		}

		function cancelModify(gwcode){
			$("#" + gwcode + "_answerForm").attr("readonly")
			$("#" + gwcode + "_answerForm").removeClass("size-110 bor8 stext-102 cl2 p-lr-20 p-tb-10");

			var output = "";
			output += '<button type="button" class="btn btn-success" onclick="modifyAnswerForm(\'' + gwcode + '\')">수정</button>';
			$("#"+gwcode+"_modifyAnswerBtn").html(output);
		}
		...
		function modifyAnswer(gwcode){
			...
			data: {"gwcode": gwcode, "gwcontents": gwcontents},
			dataType: "json",
			async: false,
			success: function(result){
				console.log(result);
